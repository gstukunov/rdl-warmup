...
export interface AllocatedRoom {
  roomNumber: number;
  openingGovernment: Array<{ telegramId: number; username: string | null; firstName: string | null; isIronman: boolean }>;
  openingOpposition: Array<{ telegramId: number; username: string | null; firstName: string | null; isIronman: boolean }>;
...
export class GameService {
  ...
  async allocatePlayers(gameId: string, initiatorTelegramId: number): Promise<AllocatedRoom[]> {
    const game = await this.getGameById(gameId);
    ...
    const initiator = game.participants?.find(p => Number(p.telegramId) === initiatorTelegramId);
    
    if (!initiator) {
      throw new ForbiddenException('Только участник может начать распределение');
    }
    ...
    if (remainingPlayers > 0) {
      ...
      if (remainingPlayers >= minPlayersForShortGame) {
        ...
        const room = this.allocateShortRoomWithIronman(fullRoomsCount + 1, remainingRoomPlayers, judges);
        rooms.push(room);
      }
    }
  ...
  private allocateRoom(roomNumber: number, players: GameParticipant[], judges: GameParticipant[], wings: GameParticipant[] = []): AllocatedRoom {
    ...
    if (players.length >= 2 && players[0].telegramId && players[1].telegramId) {
      room.openingGovernment.push(
        { telegramId: players[0].telegramId, username: players[0].username, firstName: players[0].firstName, isIronman: false },
        { telegramId: players[1].telegramId, username: players[1].username, firstName: players[1].firstName, isIronman: false }
      );
    }

    if (players.length >= 4 && players[2].telegramId && players[3].telegramId) {
      room.openingOpposition.push(
        { telegramId: players[2].telegramId, username: players[2].username, firstName: players[2].firstName, isIronman: false },
        { telegramId: players[3].telegramId, username: players[3].username, firstName: players[3].firstName, isIronman: false }
      );
    }

    if (players.length >= 6 && players[4].telegramId && players[5].telegramId) {
      room.closingGovernment.push(
        { telegramId: players[4].telegramId, username: players[4].username, firstName: players[4].firstName, isIronman: false },
        { telegramId: players[5].telegramId, username: players[5].username, firstName: players[5].firstName, isIronman: false }
      );
    }

    if (players.length >= 8 && players[6].telegramId && players[7].telegramId) {
      room.closingOpposition.push(
        { telegramId: players[6].telegramId, username: players[6].username, firstName: players[6].firstName, isIronman: false },
        { telegramId: players[7].telegramId, username: players[7].username, firstName: players[7].firstName, isIronman: false }
      );
    }
  ...
  private distributeJudges(rooms: AllocatedRoom[], judges: GameParticipant[]): void {
  ...
  private async saveAllocations(gameId: string, rooms: AllocatedRoom[]): Promise<void> {
    ...
    const allocations: RoomAllocation[] = rooms.map(room => ({
      ...
      participants: [
        ...room.openingGovernment.map(p => ({ telegramId: p.telegramId, position: 'OG', isIronman: p.isIronman })),
        ...room.openingOpposition.map(p => ({ telegramId: p.telegramId, position: 'OO', isIronman: p.isIronman })),
        ...room.closingGovernment.map(p => ({ telegramId: p.telegramId, position: 'CG', isIronman: p.isIronman })),
  ...
  async setMotionAndStart(gameId: string, motion: string, telegramId: number): Promise<Game> {
    ...
    console.log(`[setMotionAndStart] Participants:`, JSON.stringify(game.participants?.map(p => ({ id: p.id, telegramId: p.telegramId, role: p.role }))));

    // Check if user is a judge in this game
    const participant = game.participants?.find(p => Number(p.telegramId) === telegramId);
  ...
  parseScores(scoreString: string): ParsedScores {
    ...
    if (parts.length !== 2) {
      throw new BadRequestException('Invalid score format. Use "score1/score2"');
    }
  ...
  async submitScores(
    ...
    // Check if user is a judge in this game
    const judgeParticipant = game.participants?.find(p => Number(p.telegramId) === judgeTelegramId);
    ...
    if (game.status !== GameStatus.IN_PROGRESS && game.status !== GameStatus.ALLOCATING) {
    ...
    const hasOpeningGov = judgeRoom.participants.some(p => p.position === 'OG');
    const hasOpeningOpp = judgeRoom.participants.some(p => p.position === 'OO');
    const hasClosingGov = judgeRoom.participants.some(p => p.position === 'CG');
    const hasClosingOpp = judgeRoom.participants.some(p => p.position === 'CO');
    ...
    if (hasOpeningGov && openingGovernment) {
      ...
      await this.savePositionScores(gameId, judgeTelegramId, 'opening_government', ogScores, judgeRoom);
    }
    if (hasOpeningOpp && openingOpposition) {
      const ooScores = this.parseScores(openingOpposition);
      await this.savePositionScores(gameId, judgeTelegramId, 'opening_opposition', ooScores, judgeRoom);
    }
    if (hasClosingGov && closingGovernment) {
      const cgScores = this.parseScores(closingGovernment);
      await this.savePositionScores(gameId, judgeTelegramId, 'closing_government', cgScores, judgeRoom);
    }
    if (hasClosingOpp && closingOpposition) {
      const coScores = this.parseScores(closingOpposition);
      await this.savePositionScores(gameId, judgeTelegramId, 'closing_opposition', coScores, judgeRoom);
    }
  ...
  private async savePositionScores(
    ...
    position: string,
    scores: ParsedScores,
    room: { roomNumber: number; participants: Array<{ telegramId: number; position: string; isIronman: boolean }> },
  ): Promise<void> {
    // Find players in this position
    const positionAbbrev = position === 'opening_government' ? 'OG' :
                           position === 'opening_opposition' ? 'OO' :
                           position === 'closing_government' ? 'CG' : 'CO';
    
    const playersInPosition = room.participants.filter(p => p.position === positionAbbrev);
    ...
    const isIronman = playersInPosition.length === 1 && playersInPosition[0].isIronman;
  ...
  private async updateUserStats(gameId: string, room: { participants: Array<{ telegramId: number }> }): Promise<void> {
  ...
  async hasJudgeSubmittedScores(gameId: string, judgeTelegramId: number): Promise<boolean> {
  ...
  async leaveGame(gameId: string, telegramId: number): Promise<void> {
    ...
    if (game.status !== GameStatus.REGISTRATION) {
      throw new ForbiddenException('Нельзя покинуть игру после начала регистрации');
    }
  ...
  async cancelGame(gameId: string, telegramId: number): Promise<Game> {
    ...
    if (game.status !== GameStatus.REGISTRATION && game.status !== GameStatus.ALLOCATING) {
  ...
  async submitJudgeFeedback(
    ...
    if (game.status !== GameStatus.COMPLETED) {
      throw new ForbiddenException('Отзыв можно оставить только после завершения игры');
    }

    // Check if player participated in this game
    const playerParticipant = game.participants?.find(
      p => Number(p.telegramId) === playerTelegramId && p.role === ParticipantRole.PLAYER
    );
    ...
    const judgeParticipant = game.participants?.find(
      p => Number(p.telegramId) === judgeTelegramId && p.role === ParticipantRole.JUDGE
    );
    if (!judgeParticipant) {
    ...
    if (roomAllocations && roomAllocations.length > 0) {
      ...
      if (!playerRoom || !judgeRoom || playerRoom.roomNumber !== judgeRoom.roomNumber) {
        throw new ForbiddenException('Вы можете оставить отзыв только за судью из вашей комнаты');
      }
    }
  ...
  async getRoomJudges(gameId: string, playerTelegramId: number): Promise<{ telegramId: number; username: string | null; firstName: string | null }[]> {
    const game = await this.getGameById(gameId);
    ...
    const judges = game.participants?.filter(
      p => p.role === ParticipantRole.JUDGE && judgeIds.includes(Number(p.telegramId))
    ) || [];
  ...
  async getUnratedJudges(gameId: string, playerTelegramId: number): Promise<{ telegramId: number; username: string | null; firstName: string | null }[]> {
  ...
  async hasPlayerRatedJudge(gameId: string, playerTelegramId: number, judgeTelegramId: number): Promise<boolean> {
  ...
  async getJudgeAverageRating(judgeTelegramId: number): Promise<{ averageScore: number; totalFeedbacks: number }> {
   ...
   */
  async fillWithBots(gameId: string, initiatorTelegramId: number): Promise<Game> {
    const game = await this.getGameById(gameId);
    ...
    if (!initiator && !isCreator) {
      throw new ForbiddenException('Только участник или создатель игры может добавлять ботов');
    }

    // Count current players
    const currentPlayers = game.participants?.filter(p => p.role === ParticipantRole.PLAYER) || [];
    ...
    const currentJudges = game.participants?.filter(p => p.role === ParticipantRole.JUDGE) || [];
  ...
  async getUnratedJudgesWithMotion(playerTelegramId: number): Promise<{
    ...
    for (const participation of completedGames) {
      ...
      const ratedJudgeIds = new Set(ratedFeedbacks.map(f => Number(f.judgeTelegramId)));
      ...
      for (const judge of judges) {
        if (!ratedJudgeIds.has(Number(judge.telegramId))) {
          result.push({
            ...
            judgeName: judge.firstName || judge.username || `Судья ${judge.telegramId}`,
  ...
  async importGameResult(importDto: ImportGameResultDto): Promise<Game> {
    ...
    if (players.length < 2) {
      throw new BadRequestException('Minimum 2 players required (at least 1 OG and 1 OO)');
    }
    ...
    for (const player of players) {
      ...
      if (!user) {
        throw new NotFoundException(`User with telegramId ${player.telegramId} is not registered in bot. Please register first.`);
      }
    ...
    for (const judge of judges) {
      ...
      if (!user) {
        throw new NotFoundException(`Judge with telegramId ${judge.telegramId} is not registered in bot. Please register first.`);
      }
  ...
  private mapPositionToEnum(position: 'OG' | 'OO' | 'CG' | 'CO'): ParticipantPosition {
    switch (position) {
      case 'OG': return ParticipantPosition.OPENING_GOVERNMENT;
      case 'OO': return ParticipantPosition.OPENING_OPPOSITION;
      case 'CG': return ParticipantPosition.CLOSING_GOVERNMENT;
      case 'CO': return ParticipantPosition.CLOSING_OPPOSITION;
      default: return ParticipantPosition.NONE;
    }
  }

  private mapPositionToString(position: 'OG' | 'OO' | 'CG' | 'CO'): string {
    switch (position) {
      case 'OG': return 'opening_government';
      case 'OO': return 'opening_opposition';
      case 'CG': return 'closing_government';
      case 'CO': return 'closing_opposition';
      default: return 'none';
    }
  }
}
